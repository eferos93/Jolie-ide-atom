...
class JolieLanguageClient extends AutoLanguageClient {
  //cazzo metto qua?
  getGrammarScopes () { return [ 'soucer.jolie' ] }
  ...
  */
  ...
  startServerProcess () {
    atom.notifications.addError('Start Server Proc')
    console.log("Porco dioooo")
  ...
  spawnServerWithSocket () {
    ...
    return new Promise((resolve, reject) => {
      ...
      server.listen(0, 'socket://localhost', () => {
    ...
    /*return this.checkPHPVersion(command).then(() => {
      ...
      childProcess.on("close", (exitCode, signal) => {
        if(exitCode != 0 && exitCode != null) {
          atom.notifications.addError('IDE-Jolie stopped unexpectedly.')
        }
      })
